export const transparent = '0 0 0 0 transparent';
export const faint = '0 1px 0 0 rgba(22, 29, 37, 0.05)';
export const base = `0 0 0 1px rgba(63, 63, 68, 0.05),
0 1px 3px 0 rgba(63, 63, 68, 0.15)`;
export const deep = `0 0 0 1px rgba(6, 44, 82, 0.1),
  0 2px 16px rgba(33, 43, 54, 0.08)`;
export const button = '0 1px 0 rgba(0, 0, 0, 0.05)';
export const buttonFocused = '0px 0px 0px 2px rgba(106, 201, 236, 1)';
export const topBar = '0 2px 2px -1px rgba(0, 0, 0, 0.15)';
export const card = `0 0 5px rgba(23, 24, 24, 0.05), 
  0 1px 2px rgba(0, 0, 0, 0.15)`;
export const popOver = `0 3px 6px -3px rgba(23, 24, 24, 0.08), 
  0 8px 20px -4px rgba(23, 24, 24, 0.12)`;
export const layer = `0 31px 41px 0 rgba(32, 42, 53, 0.2), 
  0 2px 16px 0 rgba(32, 42, 54, 0.08)`;
export const modal = `0 26px 80px rgba(0, 0, 0, 0.2), 
  0 0px 1px rgba(0, 0, 0, 0.2)`;
export const insetButton = 'inset 0 -1px 0 rgba(0, 0, 0, 0.2)';
export const insetButtonPressed = 'inset 0px 1px 0px rgba(0, 0, 0, 0.05)';
export const focus = '0 0 0 2px rgb(106, 201, 236)';

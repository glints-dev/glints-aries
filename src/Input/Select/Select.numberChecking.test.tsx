...
describe('<Select /> can still work when receives number option children or value', () => {
  test('number option children, click option', async () => {
    const onChange = jest.fn();

    const Component = () => (
      <Select label="number option children" onChange={onChange}>
  ...
  test('number option children, search filtering', async () => {
    const Component = () => (
      <Select label="number option children">
        <Select.Option value="one">{1}</Select.Option>
        <Select.Option value="two">{2}</Select.Option>
        <Select.Option value="three">{3}</Select.Option>
      </Select>
  ...
  test('number defaultValue', async () => {
    const testNumberDefaultValue = ([defaultValue, expectedValue]: any) => {
      const Component = () => (
        <Select label="number defaultValue" defaultValue={defaultValue}>
          <Select.Option value="one">{1}</Select.Option>
          <Select.Option value="two">{2}</Select.Option>
          <Select.Option value="three">{3}</Select.Option>
        </Select>
    ...
    [
      ...
      testNumberDefaultValue([defaultValue, expectedValue]);
    });
  });

  test('number value', async () => {
    const testNumberValue = ([value, expectedValue]: any) => {
      const Component = () => (
        <Select label="number defaultValue" value={value}>
          <Select.Option value="one">{1}</Select.Option>
          <Select.Option value="two">{2}</Select.Option>
          <Select.Option value="three">{3}</Select.Option>
        </Select>
    ...
    [
      ...
      testNumberValue([value, expectedValue]);
    });
  });

  test('rerender with number value', async () => {

...
class Datepicker extends React.Component<Props, State> {
  ...
  handleOnClickSelectedDate = (date: number) => {
    ...
    const tempDate =
      Number(date.toString().length) === 1 ? `0${date}` : `${date}`;
    const tempMonth =
      Number((monthValue + 1).toString().length) === 1
        ? `0${monthValue + 1}`
        : `${monthValue + 1}`;
    const activeDate = `${currentYear}${monthValue}${date}`;

    this.setState({
      getFullDate: moment(`${currentYear}-${tempMonth}-${tempDate}`).format(
        'LL'
      ),
  ...
  static getDerivedStateFromProps(nextProps: Props, prevState: State) {
    if (
      prevState.isDatepickerFocus &&
      prevState.previousFullDate === prevState.getFullDate
    ) {
      return { isOpen: true };
    }
      ...
      tempArray.push(
        ...
        >
          <HoverContent
            ...
            currentActiveDate={
              `${currentYear}${currentMonth}${i}` === currentActiveDate &&
              currentActiveDate
            }
            hoverAble
          >
  ...
  renderDayRow = (
    ...
    functionArray.push(
      <tr role="row" key={`row${functionArray.length}`}>
        {tempArray}
      </tr>
    );
    if (isLastDate) {
          ...
          {tempMonth.slice(i * 3, 3 + i * 3).map(data => (
            <Td role="gridcell" key={data}>
              <HoverContent
                hoverAble
                biggerSize
                onMouseDown={e => this.handleChooseMonth(e, data, tempMonth)}
              >
                {data}
              </HoverContent>
            </Td>
          ...
          <DatepickerSection border>
            <DatepickerContent justify="space-between">
              ...
              <DatepickerContent>
                ...
                <DatepickerMonthYearBtn>{currentYear}</DatepickerMonthYearBtn>
              </DatepickerContent>
